<div class="space-y-8">
    @if($statusMessage)
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3.5 text-sm font-semibold text-emerald-900">
            {{ $statusMessage }}
        </div>
    @endif
    @if($errorMessage)
        <div class="rounded-xl border border-red-200 bg-red-50 px-4 py-3.5 text-sm font-semibold text-red-900">
            {{ $errorMessage }}
        </div>
    @endif
    ...
    <div>
        <input type="search" wire:model.live.debounce.300ms="search" placeholder="Search name or category…"
               class="w-full max-w-md rounded-xl border-gray-200 py-2.5 text-sm shadow-sm focus:border-middo-orange focus:ring-middo-orange">
    </div>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 text-left text-xs font-bold uppercase tracking-wide text-gray-500">
                <tr>
                    ...
                    <th class="px-4 py-3.5">Calc</th>
                    <th class="px-4 py-3.5">Scope</th>
                ...
                @forelse($charges as $charge)
                    <tr wire:key="charge-{{ $charge->id }}" class="hover:bg-gray-50/80">
                        <td class="px-4 py-3.5">
                            ...
                            <div class="text-xs text-gray-500 capitalize mt-0.5">{{ $charge->category }}</div>
                        </td>
                        <td class="px-4 py-3.5 font-semibold text-gray-900">৳{{ number_format($charge->amount) }}</td>
                        <td class="px-4 py-3.5 text-gray-600">{{ str_replace('_', ' ', $charge->calculation) }}</td>
                        <td class="px-4 py-3.5 text-gray-600 text-xs space-y-0.5">
                            <div>{{ $charge->area?->name ?? 'All areas' }}</div>
                            <div>{{ $charge->menuItem?->name ?? 'All menus' }}</div>
                        </td>
                        <td class="px-4 py-3.5 capitalize text-gray-600">{{ $charge->applies_to }}</td>
                        <td class="px-4 py-3.5">
                            @if($charge->is_active)
                                <span class="inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">Active</span>
                            @else
                                <span class="inline-flex rounded-full bg-gray-100 px-2.5 py-1 text-xs font-bold text-gray-500">Off</span>
                            @endif
                        </td>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-5 space-y-4">
        ...
        @forelse($recentUsages as $usage)
            <div class="flex items-start justify-between gap-3 text-sm border-b border-gray-50 pb-3 last:border-0 last:pb-0">
    ...
    @if($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" wire:click.self="closeForm">
            <div class="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-xl p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold uppercase text-gray-500">Name</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold uppercase text-gray-500">Category</label>
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold uppercase text-gray-500">Amount (৳)</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold uppercase text-gray-500">Calculation</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold uppercase text-gray-500">Area (optional)</label>
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold uppercase text-gray-500">Menu (optional)</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold uppercase text-gray-500">Applies to</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold uppercase text-gray-500">Starts</label>
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold uppercase text-gray-500">Ends</label>
                    ...
                    <label class="inline-flex items-center gap-2.5 text-sm font-semibold text-gray-700 pt-1">
                ...
                <div class="flex justify-end gap-3 pt-1">
                    <button type="button" wire:click="closeForm" class="rounded-xl px-4 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-100">Cancel</button>
                    <button type="button" wire:click="save" class="rounded-xl bg-middo-orange px-4 py-2.5 text-sm font-bold text-white hover:bg-[#733614]">Save</button>
                </div>
            </div>
        </div>
    @endif
