        ...
        <div wire:key="wallet-top-up-modal-root" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white w-full max-w-md rounded-2xl shadow-2xl border border-[#EBE3D3] max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between p-5 border-b border-[#EBE3D3] sticky top-0 bg-white rounded-t-2xl">
                    <div>
                        ...
                        <p class="text-xs font-semibold text-[#635347] mt-0.5">
                            Current balance:
                            <span class="font-mono text-[#1E4630]">৳{{ number_format(auth()->user()->fresh()->balance ?? 0, 2) }}</span>
                        </p>
                    </div>
                ...
                <div class="p-5 space-y-4">
                    ...
                    @if($paymentUrl)
                        <p class="text-sm text-[#635347] font-semibold">
                            Complete payment in the Middo checkout window, then confirm below.
                        </p>
                        <a href="{{ $paymentUrl }}" target="_blank" rel="noopener"
                           class="block w-full text-center bg-middo-orange hover:bg-[#733614] text-white font-black text-xs uppercase tracking-wider px-6 py-3 rounded-xl shadow-sm transition-colors">
                            Open payment gateway
                        </a>
                        <button type="button" wire:click="refreshAfterPayment"
                                class="w-full text-xs font-black text-[#1E4630] bg-[#EFE9DC] hover:bg-[#E5DCB9] py-3 rounded-xl transition">
                        ...
                        <form wire:submit="topUp" class="space-y-4">
                            ...
                            <div class="flex items-center justify-end gap-2 pt-3 border-t border-dashed border-gray-100">
                                ...
                                <button type="submit"
                                    ...
                                    <span wire:loading wire:target="topUp">Preparing...</span>
                                </button>
                            </div>
                        </form>
                    @endif
