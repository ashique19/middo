<div class="max-w-7xl mx-auto py-6 sm:py-10 px-4 sm:px-6 space-y-5 sm:space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="space-y-1">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-middo-dark">Last month</h1>
    ...
    @if($viewMode === 'list')
        <x-operation.orders.table :orders="$flatOrders" :show-group="true" :hide-customer-pii="true" empty-message="No orders last month." />
        @if($groups->hasPages())
    ...
    @forelse($groupNodes as $group)
        <div
            wire:key="kitchen-last-month-group-{{ $group['id'] }}"
            ...
            @if(count($group['orders']) > 0)
                <ul class="divide-y divide-black/5">
                    @foreach($group['orders'] as $order)
                        <li
                            wire:key="kitchen-last-month-order-{{ $group['id'] }}-{{ $order['id'] }}"
    ...
    @empty
        <div class="bg-white border border-gray-200 rounded-2xl p-12 text-center shadow-sm">
            <p class="text-sm font-semibold text-gray-400 italic">No order groups for last month.</p>
        </div>
    @endforelse
