<div class="max-w-7xl mx-auto py-10 px-6 space-y-8">
    <div class="space-y-1">
        ...
        <h1 class="text-3xl font-bold text-middo-dark">{{ $menuItem->name }}</h1>
        @if($menuItem->summary)
        ...
        <div class="space-y-3">
            @forelse($mealItems as $meal)
                <div
                    ...
                    class="bg-white border border-gray-200 rounded-2xl px-5 py-4 shadow-sm flex flex-wrap items-center gap-4 justify-between">
                    <div class="min-w-0 flex-1 space-y-1">
                        <h3 class="text-base font-bold text-middo-dark">{{ $meal['name'] }}</h3>
                        @if($meal['summary'])
                    ...
                    @if($meal['has_recipe'])
                        <a
                            ...
                            class="inline-flex items-center px-4 py-2 rounded-xl bg-middo-orange hover:bg-[#733614] text-white text-sm font-bold transition shrink-0">
                            View recipe
                        </a>
                    @endif
                </div>
